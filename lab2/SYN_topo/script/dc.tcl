...
# link
# compile
# read_saif -auto_map_names -input ../SIM/FUNCTION/${DESIGN_NAME}.saif -instance TB_${DESIGN_NAME}/U${DESIGN_NAME}
...
##################################################################################
# Lab : Start 
# At the first source MCMM scenarios, the first scenario couldn't have the dedicated constarint and it looks like bad situation 
# We should adopt all scenario files one more time with read_sdc
##################################################################################
...
if {[shell_is_in_topographical_mode]} {
  ...
  compile_ultra -spg -check_only > ${REPORTS_DIR}/${DESIGN_NAME}.compile_ultra_check_only.rpt
}
...
compile_ultra -spg > ${REPORTS_DIR}/${DESIGN_NAME}.compile_ultra.rpt
...
# compile_ultra -incremental -spg -no_autoungroup > ${REPORTS_DIR}/09_${DESIGN_NAME}_compile_ultra_incremental.rpt
...
if {[shell_is_in_topographical_mode]} {
  ...
  foreach scenario [all_active_scenarios] {
    ...
    ### smkcow ####
    ###############
    # It doesn't need read_sdc. This is 2nd time to adjust so DC already knows all constraints but this is final writing and it activates all scenarios and writes out so remind again and again below line and Users should do Lab : Start 
    #read_sdc [dcrm_mcmm_filename ${DCRM_SDC_INPUT_FILE} ${scenario}]
    ...
    # Below line was already adjusted at mcmm.scenarios.tcl
...
# Use SAIF file for power analysis
 ...
 foreach scenario [all_active_scenarios] {
   ...
   # It doesn't need read_saif. This is 2nd time to adjust 
   ...
   # smkcow! other modules are needed!! 
#   read_saif -auto_map_names -input (fifo)../SIM/FUNCTION/${DESIGN_NAME}.saif -instance TB_${DESIGN_NAME}/U${DESIGN_NAME} -verbose
#   read_saif -auto_map_names -input (memory_wrapper)../SIM/FUNCTION/${DESIGN_NAME}.saif -instance TB_${DESIGN_NAME}/U${DESIGN_NAME} -verbose
